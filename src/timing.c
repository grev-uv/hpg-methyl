...
void timing_display(timing_t* t_p) {
  // Assume that the last time on the list
  // is the total execution time
  float total_time = t_p->section_times[t_p->num_sections - 1];
  ...
  for (int i = 0; i < t_p->num_sections - 1; i++) {
    // Print the elapsed time for each section and the contribution
    // to the complete execution time
    printf("%s\t: %4.04f sec (%f%% of total)\n", t_p->section_labels[i], t_p->section_times[i] / 1000000, 
           100.0f * (t_p->section_times[i] / total_time));
  }

  printf("-----------------------------------------------------------\n");

  // Finally, print the total execution time
  printf("%s\t: %4.04f sec\n", t_p->section_labels[t_p->num_sections - 1], total_time / 1000000);
